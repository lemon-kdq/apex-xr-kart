...
static int32_t app_handle_input(android_app* app, AInputEvent* event) {
    if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_KEY &&
        ...
        if (AKeyEvent_getAction(event) == AKEY_EVENT_ACTION_UP) {
            static_cast<AndroidAppState*>(app->userData)->ExitRequested = true;
        }
        return 1;
    }
    return 0;
        ...
        while (app->destroyRequested == 0) {
            ...
            for (;;) {
                ...
                const int timeoutMilliseconds =
                    (!appState.Resumed && (!program || !program->IsSessionRunning()) && app->destroyRequested == 0) ? -1 : 0;
            ...
            if (appState.ExitRequested) {
                Log::Write(Log::Level::Info, "Releasing XR session on Android exit");
                program.reset();
                graphicsPlugin.reset();
