...
static const char* FragmentShaderGlsl = R"_(#version 320 es
    ...
    uniform lowp vec3 EffectTint;
    uniform lowp float EffectMode;

    void main() {
       FragColor = vec4(mix(PSVertexColor, EffectTint*(.65+.35*max(max(PSVertexColor.r,PSVertexColor.g),PSVertexColor.b)),EffectMode), 1);
    }
    )_";
...
struct OpenGLESGraphicsPlugin : public IGraphicsPlugin {
    ...
    ~OpenGLESGraphicsPlugin() override {
        ...
        glDeleteBuffers(2,m_sceneBuffers);
        glDeleteVertexArrays(2,m_sceneArrays);
    ...
    void InitializeResources() {
        ...
        glGenBuffers(2, m_sceneBuffers);
        glGenVertexArrays(2, m_sceneArrays);
        for(int i=0;i<2;++i){
            glBindVertexArray(m_sceneArrays[i]);
            glBindBuffer(GL_ARRAY_BUFFER,m_sceneBuffers[i]);
            glBufferData(GL_ARRAY_BUFFER,i==0?sizeof(kWorldVertices):sizeof(kCarVertices),i==0?kWorldVertices:kCarVertices,GL_STATIC_DRAW);
            glEnableVertexAttribArray(m_vertexAttribCoords);
            glEnableVertexAttribArray(m_vertexAttribColor);
            glVertexAttribPointer(m_vertexAttribCoords,3,GL_FLOAT,GL_FALSE,6*sizeof(float),nullptr);
            glVertexAttribPointer(m_vertexAttribColor,3,GL_FLOAT,GL_FALSE,6*sizeof(float),(void*)(3*sizeof(float)));
        }
    ...
    void RenderView(const XrCompositionLayerProjectionView& layerView, const XrSwapchainImageBaseHeader* swapchainImage,
        ...
        glUniform1f(glGetUniformLocation(m_program,"EffectMode"),0);
        // The same animation time is supplied once per OpenXR frame for both eyes.
        const float angle=cubes.empty()?0.f:cubes[0].Pose.position.x;
        for(int part=0;part<2;++part){
            const float unit=.00675f;
            XrVector3f position{0,-.22f,-1.5f};
            XrQuaternionf rotation{0,0,0,1};
            if(part==1){
                if(!cubes.empty()){
                    position.x+=cubes[0].Pose.position.x*unit;
                    position.z+=cubes[0].Pose.position.z*unit;
                    rotation=cubes[0].Pose.orientation;
                }
            }
            XrVector3f size{unit,unit,unit};
            XrMatrix4x4f model,mvp;
            XrMatrix4x4f_CreateTranslationRotationScale(&model,&position,&rotation,&size);
            XrMatrix4x4f_Multiply(&mvp,&vp,&model);
            glUniformMatrix4fv(m_modelViewProjectionUniformLocation,1,GL_FALSE,reinterpret_cast<const GLfloat*>(&mvp));
            glBindVertexArray(m_sceneArrays[part]);
            glDrawArrays(GL_TRIANGLES,0,part==0?sizeof(kWorldVertices)/(6*sizeof(float)):sizeof(kCarVertices)/(6*sizeof(float)));
        }
        glUniform1f(glGetUniformLocation(m_program,"EffectMode"),1);
        glFrontFace(GL_CW);
        glBindVertexArray(m_vao);
        for(size_t i=1;i<cubes.size();++i){const auto& c=cubes[i];const float unit=.00675f;
            XrVector3f pos{c.Pose.position.x*unit,c.Pose.position.y*unit-.22f,c.Pose.position.z*unit-1.5f};
            XrVector3f size{c.Scale.x*unit,c.Scale.y*unit,c.Scale.z*unit};XrMatrix4x4f model,mvp;
            XrMatrix4x4f_CreateTranslationRotationScale(&model,&pos,&c.Pose.orientation,&size);
            XrMatrix4x4f_Multiply(&mvp,&vp,&model);
            glUniformMatrix4fv(m_modelViewProjectionUniformLocation,1,GL_FALSE,reinterpret_cast<const GLfloat*>(&mvp));
            glUniform3f(glGetUniformLocation(m_program,"EffectTint"),c.Color.x,c.Color.y,c.Color.z);
   ...
   private:
    ...
    GLuint m_sceneBuffers[2]{};
    GLuint m_sceneArrays[2]{};
    GLuint m_vao{0};
