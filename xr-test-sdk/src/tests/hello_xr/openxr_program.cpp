...
struct OpenXrProgram : IOpenXrProgram {
    ...
    void CreateSwapchains() override {
        ...
        if (viewCount > 0) {
            ...
            for (uint32_t i = 0; i < viewCount; i++) {
                ...
                XrResult createResult = XR_ERROR_SESSION_NOT_READY;
                for (int attempt = 0; attempt < 100; ++attempt) {
                    createResult = xrCreateSwapchain(m_session, &swapchainCreateInfo, &swapchain.handle);
                    if (createResult != XR_ERROR_SESSION_NOT_READY) break;
                    std::this_thread::sleep_for(std::chrono::milliseconds(100));
                }
                CHECK_XRRESULT(createResult, "xrCreateSwapchain after service startup wait");
    ...
    void PollEvents(bool* exitRenderLoop, bool* requestRestart) override {
        *exitRenderLoop = *requestRestart = false;
        m_kart.update();
        if(m_kart.exit){*exitRenderLoop=true;return;}
    ...
    bool RenderLayer(XrTime predictedDisplayTime, std::vector<XrCompositionLayerProjectionView>& projectionLayerViews,
        ...
        cubes.push_back(Cube{{{0,std::sin(m_kart.yaw/2),0,std::cos(m_kart.yaw/2)},{m_kart.x,0,m_kart.z}},{1,1,1}});

        m_kart.nitro.draw(cubes,m_kart.x,m_kart.z,m_kart.yaw);
        // Six digit pairing code, rendered in world space above the front edge.
        static const char* digits[]={"111101101101111","010110010010111","111001111100111","111001111001111","101101111001001","111100111001111","111100111101111","111001001001001","111101111101111","111101111001111"};
        for(size_t d=0;d<m_kart.remote.pairCode.size();++d){int digit=m_kart.remote.pairCode[d]-'0';if(digit<0||digit>9)continue;
            for(int row=0;row<5;++row)for(int col=0;col<3;++col)if(digits[digit][row*3+col]=='1'){
                cubes.push_back(Cube{{{0,0,0,1},{-9.f+float(d)*3.2f+col*.7f,7.f-row*.7f,27.f}},{.55f,.55f,.12f},{1.f,.85f,.3f}});
            }
        }
